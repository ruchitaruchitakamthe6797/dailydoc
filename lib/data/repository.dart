...
class Repository {
  ...
  Future<ChatDetailsResponse> getConversationList(String params) async {
    ...
    // later use
    return await _postApi.getConversationList(params).then((postsList) {
      // postsList.data!.messages!.forEach((messages) {
      //   _postDataSource.insert(messages);
      // });
  ...
  Future<ChatListResponse> getChatList() async {
    ...
    // else make a network call to get all posts, store them into database for
    // later use
    return await _postApi.getChatList().then((postsList) {
  ...
  Future<SendMessageResponse> sendMessage(
      String params, String message, String id) async {
    // check to see if posts are present in database, then fetch from database
    // else make a network call to get all posts, store them into database for
    // later use
    return await _postApi.sendMessage(params, message, id).then((postsList) {
...
*/
  ...
  Future<List<ChatListData>> getChatListDb(int id) {
    //creating filter
    List<Filter> filters = [];
    ...
    //making db call
    return _postDataSource
        .getChatList(filters: filters)
        .then((posts) => posts)
        .catchError((error) => throw error);
  }

  Future<int> insert(ChatListData post) => _postDataSource
      .insert(post)
      .then((id) => id)
      .catchError((error) => throw error);
